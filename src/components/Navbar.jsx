...
import {
  Menu, X, ChevronDown, LogOut, LayoutDashboard, FileText,
  Settings, Users, BookOpen, Shield, Star, ChevronRight, Sparkles,
} from "lucide-react";
  ...
  return (
    <>
      <style>{`
        ...
        /* Archives dropdown trigger */
        .nb-arc-wrap { position:relative; }
        .nb-arc-btn {
          position:relative; padding:7px 12px; font-size:13.5px; font-weight:500;
          color:#4b5563; border-radius:8px; text-decoration:none;
          transition:color 0.15s, background 0.15s; white-space:nowrap;
        ...
        @media(min-width:1200px) { .nb-arc-btn { padding:7px 15px; font-size:14px; } }
        .nb-arc-btn:hover, .nb-arc-btn.arc-active { color:#1d4ed8; background:#eff6ff; }
        .nb-arc-btn.arc-active::after {
          content:''; position:absolute; bottom:5px; left:50%; transform:translateX(-50%);
          width:5px; height:5px; border-radius:50%; background:#2563eb;
        }
        .nb-arc-chevron { transition:transform 0.2s; }
        .nb-arc-chevron.open { transform:rotate(180deg); }

        /* Archives dropdown panel */
        .nb-arc-panel {
          position:absolute; top:calc(100% + 8px); left:0;
          background:white; border:1px solid #e5e7eb; border-radius:14px;
          box-shadow:0 12px 40px rgba(0,0,0,0.09), 0 2px 10px rgba(0,0,0,0.04);
          overflow:hidden; min-width:230px; z-index:200;
          animation:nbDrop 0.18s cubic-bezier(0.34,1.56,0.64,1);
        }
        @keyframes nbDrop { from{opacity:0;transform:scale(0.93) translateY(-6px);} to{opacity:1;transform:scale(1) translateY(0);} }
        ...
        .nb-mob-arc-child {
          ...
          font-size:13.5px; font-weight:500; color:#6b7280; border-radius:8px;
          text-decoration:none; transition:all 0.14s;
        }
        .nb-mob-arc-child:hover { background:#eff6ff; color:#1d4ed8; }
      `}</style>
        ...
        <div className="nb-inner">
          ...
          <nav className="nb-links">
            ...
            <div className="nb-arc-wrap" ref={archivesRef}>
              ...
              {archivesOpen && (
                <div className="nb-arc-panel">
                  <div className="nb-dd-sec">
                    {archiveLinks.map(({ to, label, icon }) => (
                      <Link key={to} to={to} className="nb-dd-item">
            ...
            {user && (
              <NavLink to="/submit" className={({ isActive }) => `nb-link${isActive ? " active" : ""}`}>
                Submit
              </NavLink>
            )}
          </nav>
        ...
        {mobileOpen && (
          <div className="nb-mobile">
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <NavLink to="/" end className={({ isActive }) => `nb-mob-link${isActive ? " active" : ""}`}>
                Home
              </NavLink>

              {/* Archives section in mobile */}
              <div>
                <div
                  className="nb-mob-link"
                  style={{ color: location.pathname.startsWith("/archives") ? "#1d4ed8" : undefined,
                           background: location.pathname.startsWith("/archives") ? "#eff6ff" : undefined }}
                >
                  <BookOpen size={15} /> Archives
                </div>
                {archiveLinks.map(({ to, label, icon }) => (
                  <Link key={to} to={to} className="nb-mob-arc-child">
                    {icon} {label}
                  </Link>
                ))}
              </div>
              ...
              {links.slice(1).map(({ to, label }) => (
                <NavLink key={to} to={to} className={({ isActive }) => `nb-mob-link${isActive ? " active" : ""}`}>
                  {label}
                </NavLink>
              ))}

              {user && (
                <NavLink to="/submit" className={({ isActive }) => `nb-mob-link${isActive ? " active" : ""}`}>
              ...
              {user ? (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #f1f5f9", display: "flex", flexDirection: "column", gap: 2 }}>
                  <div style={{
                    ...
                    border: "1px solid #e0e7ff", borderRadius: 12, marginBottom: 6,
                  }}>
                    <div
                      className="nb-avatar"
                      style={{
                  ...
                  <Link to={dashPath} className="nb-mob-link">
                    <DashIcon size={15} /> {isAdmin ? "Admin Panel" : "Dashboard"}
                  </Link>
                  {!isAdmin && (
                  ...
                  {isAdmin && (
                    <>
                      <Link to="/dashboard/admin/manage-reviewers" className="nb-mob-link">
                        <Users size={15} /> Manage Reviewers
                      </Link>
                      <Link to="/dashboard/admin/assign-reviewers" className="nb-mob-link">
                        <Sparkles size={15} /> Assign Reviewers
                      </Link>
                      <Link to="/dashboard/admin/review-decisions" className="nb-mob-link">
                ...
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #f1f5f9", display: "flex", flexDirection: "column", gap: 8 }}>
                  <Link to="/login" className="nb-mob-link" style={{ justifyContent: "center", border: "1.5px solid #e5e7eb" }}>
                    Login
                  </Link>
